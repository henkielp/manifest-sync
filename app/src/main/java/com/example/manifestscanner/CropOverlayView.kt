...
class CropOverlayView @JvmOverloads constructor(
    ...
    private fun hitTest(x: Float, y: Float): DragHandle {
        ...
        // Interior: move the entire rectangle
    ...
    private fun applyDrag(handle: DragHandle, dx: Float, dy: Float) {
        when (handle) {
           ...
           DragHandle.MOVE -> {
                // Move entire rectangle, clamping to view bounds.
                var newLeft = cropRect.left + dx
                var newTop = cropRect.top + dy
                val rectW = cropRect.width()
                val rectH = cropRect.height()

                newLeft = newLeft.coerceIn(0f, width.toFloat() - rectW)
                newTop = newTop.coerceIn(0f, height.toFloat() - rectH)

                cropRect.set(newLeft, newTop, newLeft + rectW, newTop + rectH)
            }
